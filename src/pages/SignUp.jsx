...
const Signup = () => {
  ...
  const cms = useCMS('auth_signup', {});
  const pageTitle = cms.title || 'Join Us';
  const pageSubtitle = cms.subtitle || 'Create your sanctuary account';
  ...
  const allValid = validations.every(v => v.test) && gender !== '' && ageRange !== '' && fullName.length >= 2 && email.includes('@') && phone.trim().length >= 7 && country.trim().length >= 2;
  ...
  const handleCreateAccount = async (e) => {
    ...
    setLoading(true);
    try {
      await register({ fullName, email, password, phone, country, gender, ageRange });
      navigate('/profile');
    } catch (err) {
  ...
  const styles = {
    page: { minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', padding: '180px 28px 60px', fontFamily: "'Encode Sans Expanded', sans-serif" },
    ...
    input: { width: '100%', padding: '15px 40px 15px 0', border: 'none', borderBottom: '2px solid #112a3d', background: 'transparent', outline: 'none', fontSize: '1.1rem', color: '#112a3d', fontWeight: '600', boxSizing: 'border-box' },
      ...
      <div style={styles.formWrapper}>
        ...
        <form onSubmit={handleCreateAccount}>
          ...
          <div style={styles.inputGroup}>
            ...
            <input type="tel" style={styles.input} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+20 123 456 789" required />
          </div>

          <div style={styles.inputGroup}>
            <label style={styles.label}>Country <span style={{ color: '#c0392b' }}>*</span></label>
            <input type="text" style={styles.input} value={country} onChange={(e) => setCountry(e.target.value)} placeholder="Egypt" required />
          </div>
