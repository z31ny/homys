...
const AdminSideBar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Website Content', icon: <Globe         size={22} />, path: '/admin/website-content' },
    { name: 'Notifications',   icon: <Bell          size={22} />, path: '/admin/notifications' },
    { name: 'Settings',        icon: <Settings      size={22} />, path: '/admin/settings' },
  ];

  const initials = (name) => (name || 'AD').split(' ').map((w) => w[0]).join('').toUpperCase().slice(0, 2);
