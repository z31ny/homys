...
import { Calendar, Star, Home, MessageSquare, ChevronRight } from 'lucide-react';
...
const Notifications = () => {
  ...
  useEffect(() => {
    // Mark notifications as seen so the bell badge clears.
    localStorage.setItem('homys_notif_last_seen', String(Date.now()));
    Promise.all([
      adminAPI.getBookings({ limit: 10 }),
      adminAPI.getPendingReviews(),
      adminAPI.getProperties({ status: 'pending_review', limit: 10 }),
      adminAPI.getContacts({ limit: 10 }).catch(() => ({ data: {} })),
    ])
      .then(([bookingsRes, reviewsRes, propsRes, contactsRes]) => {
        const bookingEvents = (bookingsRes.data.bookings || []).map((b) => ({
          ...
          desc: `${b.guestFirstName || ''} ${b.guestLastName || ''} · ${b.checkIn} → ${b.checkOut} · EGP ${b.totalPrice} · ${b.status}`,
          time: b.createdAt,
          filter: 'Bookings',
          link: '/admin/bookings',
        }));
        ...
        const contactEvents = (contactsRes.data?.contacts || []).map((c) => ({
          id: `c-${c.id}`,
          type: 'message',
          title: `New Message — ${c.name || 'Guest'}`,
          desc: (c.message || '').slice(0, 80) + ((c.message || '').length > 80 ? '…' : ''),
          time: c.createdAt,
          filter: 'Messages',
          link: '/admin/messages',
        }));

        const all = [...bookingEvents, ...reviewEvents, ...propEvents, ...contactEvents].sort(
  ...
  const filters = ['All', 'Bookings', 'Reviews', 'Properties', 'Messages'];
        ...
        {loading ? (
          ...
          <section className="notif-section">
            <div className="notif-section-card">
              {filtered.map((item) => (
                <div key={item.id} className="notif-item notif-item-clickable" onClick={() => item.link && navigate(item.link)}>
